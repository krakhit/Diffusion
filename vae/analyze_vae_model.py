...
# Load the trained VAE model specifically (most recent VAE training)
...
print(f"Loading model from: my_model/{last_training}")
...
# Encode to latent space - VAE returns distribution, so we take the mean
with torch.no_grad():
...
ax1.set_title('VAE Latent Space Distribution', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('vae_latent_space_analysis.png', dpi=150, bbox_inches='tight')
plt.close()
...
if latent_dim > 2:
    ...
    ax.set_title('VAE Latent Space PCA Projection (2D)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('vae_latent_space_pca.png', dpi=150, bbox_inches='tight')
    plt.close()
...
if latent_dim > 2:
    ...
    scatter = ax.scatter(latent_tsne[:, 0], latent_tsne[:, 1], 
                        c=sample_labels, cmap='tab10', alpha=0.6, s=20)
    ax.set_title('VAE Latent Space t-SNE Projection (2D)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('vae_latent_space_tsne.png', dpi=150, bbox_inches='tight')
    plt.close()

print("\n✓ VAE Analysis complete!")
print("\nGenerated files:")
print("  - vae_latent_space_analysis.png (comprehensive analysis)")
if latent_dim > 2:
    print("  - vae_latent_space_pca.png (PCA projection)")
    print("  - vae_latent_space_tsne.png (t-SNE projection)")
